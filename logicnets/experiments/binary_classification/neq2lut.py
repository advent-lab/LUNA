...
from models import QuantumNeqModel, QuantumLutModel
import numpy as np

def calculate_fidelity_from_cm(confusion_matrix):
    # True negatives (benign samples correctly classified)
    true_negatives = confusion_matrix[0, 0]
    
    # False positives (attack samples classified as benign)
    false_positives = np.sum(confusion_matrix[1:, 0])
    
    # False negatives (benign samples classified as attack)
    false_negatives = np.sum(confusion_matrix[0, 1:])
    
    # True positives (attack samples correctly classified)
    true_positives = np.sum(confusion_matrix[1:, 1:])
    
    # Calculate precision and recall
    
    fidelity = 1 - 0.5*((false_negatives/(false_negatives+true_positives) + false_positives/(false_positives+true_negatives)))
    # Calculate F1-score (Detection Score)
...
if __name__ == "__main__":
    ...
    val_detection_score = calculate_fidelity_from_cm(val_cm)
    print("Val Fidelity: %.5f" % val_detection_score)
    ...
    #print(test_cm)
    test_detection_score = calculate_fidelity_from_cm(test_cm)
    print("Test Fidelity: %.5f" % test_detection_score)
    
    # Instantiate LUT-based model
    lut_model = QuantumLutModel(model_cfg)
    ...
    print("LUT-Based Model accuracy: %f" % (test_accuracy))
    ...
    ret = synthesize_and_get_resource_counts(options_cfg["log_dir"], "logicnet", fpga_part="xczu7ev-ffvc1156-2-i", clk_period_ns=args.clock_period, post_synthesis = 1)
    print(ret)
