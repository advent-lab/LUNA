...
# Configuration based on `area-opt` architecture
NUM_TESTS = 10000
START = 100
FLEN = 400
NUM_FILTER = 1  # 1 Window
SHIFT_M = 9
SHIFT_N = 0
IQ_WIDTH_IN = 14
    ...
    with open(os.path.join(data_out, "axis_stimulus.hex"), "w") as f_axis, \
        ...
        for i in range(NUM_TESTS):
            ...
            for w in range(NUM_FILTER):
                ...
                acc_I_final = limit_width(acc_I >> SHIFT_N, 14)
                acc_Q_final = limit_width(acc_Q >> SHIFT_N, 14)
                sum_I.append(acc_I_final)
                sum_Q.append(acc_Q_final)

            # feature_bw for area-opt: feature_bitwidth(14,400,1,9,0) = 14 bits per feature
            # Training order: [I0, Q0] mapped via features_to_bitarray (MSB-first per feature)
            FEAT_BW = 14
            logicnet_word = (
                bitrev(sum_I[0], FEAT_BW)        |   # M0[0:14]   = bitrev(I0)
                (bitrev(sum_Q[0], FEAT_BW) << 14)    # M0[14:28]  = bitrev(Q0)
            )
            f_logicnet.write(f"{logicnet_word:07X}\n")
