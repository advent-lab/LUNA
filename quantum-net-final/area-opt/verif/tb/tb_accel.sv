...
module tb_accel;
    ...
    reg [27:0] logicnet_mem [0 : N_SAMPLES - 1];
    ...
    initial begin
        ...
        for (int test = 0; test < N_SAMPLES; test = test + 1) begin
            ...
            // 3. Wait for adder tree latency (sum_signed has 9 stages, so 9 clock cycles)
            repeat(9) @(posedge ap_clk);
