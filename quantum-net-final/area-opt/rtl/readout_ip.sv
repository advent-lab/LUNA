module nn_classifier_wrapper #(
    ...
    // Width of the NN prediction output (area-opt logicnets use 1 bit).
    ...
    always @(*) begin
        ...
        case (current_state)
            ...
            S_COMPUTE: begin
                // Transition after NN latency (sum_signed = 9 cycles, logicnet = 4 cycles -> total 13 cycles minimum)
                if (compute_count >= 13) 
                    next_state = S_STORE;
            end
...
module nn_accelerator #(
    ...
    // Apply SHIFT_N truncation (1 LSB dropped). Result is 14 bits wide (since SHIFT_N is 0).
    ...
    // Interleaving: Q0, I0 (28 bits total).
