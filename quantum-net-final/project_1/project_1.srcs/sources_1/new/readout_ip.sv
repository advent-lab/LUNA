            ...
            case (current_state)
                ...
                S_LOAD: begin
                    if (in_TVALID && load_count < WINDOW_SIZE) begin
                        ...
                        // I is [31:18]. Shifted slice is [31 - SHIFT_M : 18]
                        // 31-7=24, so in_TDATA[24:18]
                        i_memory[load_count] <= in_TDATA[IQ_WIDTH_IN + IQ_WIDTH_IN - SHIFT_M - 1 : IQ_WIDTH_IN];
                        
                        // Q value storage (shifted)
                        // Q is [17:4]. Shifted slice is [17 - SHIFT_M : 4]
                        // 17-7=10, so in_TDATA[10:4]
                        q_memory[load_count] <= in_TDATA[IQ_WIDTH_IN - SHIFT_M - 1 : 4];
...
module nn_accelerator #(
    ...
    localparam ACCEL_NUM_WINDOWS = 2; 
    ...
    localparam LOGICNET_IN_WIDTH = 300;                               // From logicnet IP definition
