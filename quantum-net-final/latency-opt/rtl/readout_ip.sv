...
module nn_classifier_wrapper #(
    ...
    always @(*) begin
        ...
        case (current_state)
            ...
            S_COMPUTE: begin
                // Transition after NN latency (sum_signed = 8 cycles, logicnet = 4 cycles -> total 12 cycles minimum)
                if (compute_count >= 12) 
                    next_state = S_STORE;
            end
...
module nn_accelerator #(
    ...
    // Feature ordering and Bit-Reversal:
    // LogicNet model expects the bits in training order (I0, I1, Q0, Q1),
    // meaning I0 goes into the lowest bits of M0.
    wire [FINAL_SUM_WIDTH-1:0] rev_q1;
    wire [FINAL_SUM_WIDTH-1:0] rev_q0;
    wire [FINAL_SUM_WIDTH-1:0] rev_i1;
    wire [FINAL_SUM_WIDTH-1:0] rev_i0;
    
    assign rev_q1 = {<<{sum_final_q1}};
    assign rev_q0 = {<<{sum_final_q0}};
    assign rev_i1 = {<<{sum_final_i1}};
    assign rev_i0 = {<<{sum_final_i0}};
