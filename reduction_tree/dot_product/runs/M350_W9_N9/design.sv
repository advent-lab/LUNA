...
module dotprod #(
    ...
    always_comb begin
        ...
        localparam int STAGE9_SIZE = (STAGE8_SIZE + 1) >> 1;
        logic [SUMW-1:0] stage9 [0:STAGE9_SIZE-1];
        always_ff @(posedge clk) begin
            if (rst) begin
                for (int k=0; k<STAGE9_SIZE; k++) stage9[k] <= '0;
            end else begin
                for (int k=0; k<STAGE9_SIZE; k++) begin
                    if (2*k+1 < STAGE8_SIZE)
                        stage9[k] <= stage8[2*k] + stage8[2*k+1];
                    else
                        stage9[k] <= stage8[2*k];
                end
            end
    ...
    assign result  = stage9[0];
    assign vld_out = vld_layer9;
